...
class MADDPGTesterAgent(TesterAgent):
    ...
    def _load_model(self, save_path):
        noise_type = 'ou'
        noise_std = np.array([0.0,0.02], dtype=np.float32)
        noise_mean = np.array([0.0, -0.4], dtype=np.float32)
        action_dim = 2
        state_shape = (84,96,3)
        ...
        self.action_divide_factor_gb = 4

        directory_to_save = os.path.dirname(os.path.dirname(os.path.dirname(__file__)))

        config_write = dict(num_cars=self.num_cars, noise_type=noise_type, noise_std=noise_std, noise_mean=noise_mean, 
                            train_or_test=train_or_test, ckpt_load=ckpt_load, action_dim=action_dim, directory_to_save=directory_to_save)
