...
#TODO: when it is multiagent, get visited_tile_anchorA and anchorB for both cars
...
K_BACKWARD = 0.0  # Penalty weight: backwards_penalty = K_BACKWARD * angle_diff  (if angle_diff > BACKWARD_THRESHOLD)
GRASS_DRIVING_PENALTY = 0.02 # penalty to each step that cars drive on grass
OFF_FIELD_PENALTY = 100
MAX_SCORE = 2000
    ...
    def _contact(self, contact, begin):
        ...
        if begin:
            ...
            if not tile.road_visited[obj.car_id]:
                ...
                self.env.tiles.append(tile)
                self.env.visited_tile_j1.append(list(tile.world.joints[0].anchorA))
                self.env.visited_tile_j2.append(list(tile.world.joints[1].anchorA))
                self.env.visited_tile_j3.append(list(tile.world.joints[2].anchorA))
                self.env.visited_tile_j4.append(list(tile.world.joints[3].anchorA))
                ...
                self.env.reward[obj.car_id] += reward_factor * MAX_SCORE/len(self.env.track)
        else:
...
class MultiCarRacing(gym.Env, EzPickle):
    ...
    def __init__(self, num_agents=2, verbose=1, direction='CCW',
        ...
        self.visited_tile_j1 = []
        self.visited_tile_j2 = []
        self.visited_tile_j3 = []
        self.visited_tile_j4 = []
        self.tiles = []
        ...
        self.fd_tile = fixtureDef(shape = polygonShape(vertices=
                                    [(0, 0),(1, 0),(1, -1),(0, -1)]))
    ...
    def reset(self):
        ...
        self.visited_tile_anchorA = []
        self.visited_tile_anchorB = []
        self.t = 0.0
        self.road_poly = []
    ...
    def step(self, action):
        ...
        if action is not None: # First step without action, called from reset()
            ...
            # print('anchor', self.visited_tile_anchorA[-1])
            # print('tiles', np.array(self.track)[:10, 2:], np.array(self.track)[:, 2:].shape)
            # print(np.array(self.track)[:, :2])
            # print(self.visited_tile_anchorA[-1] in np.array(self.track)[:, :2].tolist())
            ...
            for car_id, car in enumerate(self.cars):
                ...
                if abs(x) > PLAYFIELD or abs(y) > PLAYFIELD:
                    ...
                    self.reward[car_id] += -OFF_FIELD_PENALTY
                if self.driving_on_grass[car_id]:
                    self.reward[car_id] += -GRASS_DRIVING_PENALTY
            ...
            for car_id, car in enumerate(self.cars):  # Enumerate through cars
                ...
                # a = np.array(self.track)[:, 2:].tolist()
                # avg = (np.array(self.visited_tile_j1) + np.array(avg) + np.array(self.visited_tile_j3) + np.array(self.visited_tile_j4) )/4
                # ind_to_pop = []
                # for i in range(len(avg)):
                #     norm_to_all_tiles = np.linalg.norm(self.visited_tile_j2[i] - np.array(self.track)[:, 2:], ord=2, axis=1)
                #     track_index = np.argmin(norm_to_all_tiles)
                #     ind_to_pop.append(track_index)
                # ind_to_pop = np.unique(ind_to_pop)
                # rem_tiles = somelist = [i for j, i in enumerate(a) if j not in ind_to_pop]
