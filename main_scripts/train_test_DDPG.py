...
if __name__=="__main__":
    ...
    parser.add_argument('--max_episode_length', type=int, default=1250, help='Max number of timesteps for a single episode')
    ...
    parser.add_argument('--patience', type=int, default=4000, help='Patience for early stoping callback')

    parser.add_argument('--noise_type', type=str, default='ou', help='Type of noise to be added to the action (ou or normal)')
    parser.add_argument('--noise_std', type=str, default='0.1,0.8', help='Comma separated noise std for OU noise, check the length matches the action dimension, make sure it is close to 0 for testing')
    parser.add_argument('--noise_mean', type=str, default='0.0,0.0', help='Comma separated noise mean, check the length matches the action dimension, make sure it is close to 0 for testing')
    ...
    parser.add_argument('--tau', type=float, default=0.008, help='1 - polyak average weight to update target network weights')
    ...
    parser.add_argument('--action_divide_factor_gb', type=float, default=8, help='Divide the gas/brake action generated to ease the training')
    ...
    parser.add_argument('--decay_noise_steps', type=int, default=20, help='Exponential decay steps for noise')
    ...
    parser.add_argument('--max_to_keep', type=int, default=200, help='Maximum number of checkpoints to save')
    ...
    if train_or_test == 'train':
        ...
        for st in range(min_memory_size):
            ...
            action[0] = action[0] / action_divide_factor_steer
            action[1:] = action[1:] / action_divide_factor_gb
        ...
        for ep_num in range(start_ep, num_episodes):
            ...
            callback_stop = callback(ep_reward) 
            if callback_stop:
                break
